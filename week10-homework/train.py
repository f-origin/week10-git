...
# Perform the upsampling
upsample_filter_np_x8 = bilinear_upsample_weights(upsample_factor_x8,
                                                   number_of_classes)
#upsampling 8X  equal to input
upsample_filter_tensor_x8 = tf.Variable(upsample_filter_np_x8, name='vgg_16/fc8/t_conv_x8')
upsampled_logits = tf.nn.conv2d_transpose(upsampled_logits, upsample_filter_tensor_x8,
                                          output_shape=upsampled_logits_shape,
                                          strides=[1, upsample_factor_x8, upsample_factor_x8, 1],
                                          padding='SAME')
...
with sess:
    ...
    for i in range(FLAGS.max_steps):
        ...
        if gs % 10 == 0:
            # rename gs to gs_1,preventing variable coverage
            gs_1, loss, summary_string = sess.run([global_step, cross_entropy_loss, merged_summary_op], feed_dict=feed_dict_to_use)
